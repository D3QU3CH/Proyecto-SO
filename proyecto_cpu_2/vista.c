...
void vistaMostrarHistorialCPU(void)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        printf(VERDE "[");
        for (int j = 0; j < uso / 10; j++)         printf("#");
        printf(ROJO);
        for (int j = 0; j < desperdicio / 10; j++) printf("-");
        printf(RESET "] %d%%  ", uso);
    }
    printf("\n");
}
...
void vistaMostrarBalanceColas(Cola *colaEnCiclo, SistemaES *es)
{
    ...
    printf(NEGRITA "\n  # BALANCE DE COLAS #\n" RESET);
    printf("  Listos: " VERDE "%d" RESET "  |  En E/S: " AMARILLO "%d" RESET
           "  |  Total activos: %d\n", listos, enES, total);
    ...
    for (int i = 0; i < n - 1; i++)
        for (int j = i + 1; j < n; j++)
            if (campo(arr[j]) > campo(arr[i])) {
                Proceso *tmp = arr[i]; arr[i] = arr[j]; arr[j] = tmp;
            }
}
...
/* Estado del sistema                                                  */
/* ------------------------------------------------------------------ */
void vistaMostrarEstadoSistema(Cola *colaListos, int algoritmo,
